    ...
    def handle(self, *args, **kwargs):
        ...
        for data in user_data:
            ...
            if created:
                ...
                self.stdout.write(
                    self.style.NOTICE(f'Skipped User: {data["username"]}')
                )
            users.append(user)
        ...
        for data in country_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'Created Country: {data["name"]}')
                )
            else:
        ...
        for data in condition_data:
            condition, created = Condition.objects.get_or_create(name=data["name"])
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'Created Condition: {data["name"]}')
                )
            else:
                self.stdout.write(
                    self.style.NOTICE(f'Skipped Condition: {data["name"]}')
                )
        ...
        for data in transmission_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'Created Transmission: {data["name"]}')
                )
            else:
                self.stdout.write(
                    self.style.NOTICE(f'Skipped Transmission: {data["name"]}')
                )
        ...
        for data in fuel_type_data:
            fuel_type, created = FuelType.objects.get_or_create(name=data["name"])
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'Created FuelType: {data["name"]}')
                )
            else:
                self.stdout.write(
                    self.style.NOTICE(f'Skipped FuelType: {data["name"]}')
                )
        ...
        for data in insurance_data:
            insurance, created = Insurance.objects.get_or_create(name=data["name"])
            insurances.append(insurance)  # Make sure to append to the list
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'Created Insurance: {data["name"]}')
                )
            else:
                self.stdout.write(
                    self.style.NOTICE(f'Skipped Insurance: {data["name"]}')
                )

        # Debug output for insurances
        self.stdout.write(
            self.style.SUCCESS(f"Total insurances created: {len(insurances)}")
        )
        ...
        if len(insurances) < 2:
            raise ValueError(
                f"Not enough insurances were created. Expected at least 2, but got {len(insurances)}"
            )
        ...
        for data in payment_method_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'Created PaymentMethod: {data["name"]}')
                )
            else:
                self.stdout.write(
                    self.style.NOTICE(f'Skipped PaymentMethod: {data["name"]}')
                )

        # Debug output for payment methods
        self.stdout.write(
            self.style.SUCCESS(f"Total payment methods created: {len(payment_methods)}")
        )
        ...
        for data in option_data:
            option, created = Option.objects.get_or_create(name=data["name"])
        ...
        for data in extra_data:
            extra, created = Extra.objects.get_or_create(name=data["name"])
        ...
        for data in region_data:
            ...
            region, created = Region.objects.get_or_create(name=data["name"])
        ...
        if len(regions) < 2:
            raise ValueError(
                "Not enough regions were created. Expected at least 2 regions."
            )
        ...
        for data in region_specs_data:
            region_spec, created = RegionSpecs.objects.get_or_create(name=data["name"])
            region_specs.append(region_spec)  # Add this line to append to the list
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'Created RegionSpecs: {data["name"]}')
                )
            else:
                self.stdout.write(
                    self.style.NOTICE(f'Skipped RegionSpecs: {data["name"]}')
                )

        # Verify region_specs were created
        self.stdout.write(
            self.style.SUCCESS(f"Total region specs created: {len(region_specs)}")
        )
        ...
        if len(region_specs) < 1:
            raise ValueError(
                "No region specs were created. Expected at least 1 region spec."
            )
        ...
        for data in color_data:
            color, created = Color.objects.get_or_create(name=data["name"])
        ...
        for data in warranty_data:
            warranty, created = Warranty.objects.get_or_create(name=data["name"])
            warranties.append(warranty)  # Make sure to append to the list
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'Created Warranty: {data["name"]}')
                )
            else:
                self.stdout.write(
                    self.style.NOTICE(f'Skipped Warranty: {data["name"]}')
                )
        ...
        for data in pre_category_data:
            pre_category, created = PreCategory.objects.get_or_create(
                name=data["name"], defaults={"title": data["title"]}
            )
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'Created PreCategory: {data["name"]}')
                )
            else:
                self.stdout.write(
                    self.style.NOTICE(f'Skipped PreCategory: {data["name"]}')
                )
        ...
        for data in image_group_name_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'Created ImageGroupName: {data["name"]}')
                )
            else:
                self.stdout.write(
                    self.style.NOTICE(f'Skipped ImageGroupName: {data["name"]}')
                )
        ...
        for data in category_data:
            ...
            if created:
                ...
                self.stdout.write(
                    self.style.NOTICE(f'Skipped Category: {data["name"]}')
                )
        ...
        for data in subcategory_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'Created Subcategory: {data["name"]}')
                )
            else:
                self.stdout.write(
                    self.style.NOTICE(f'Skipped Subcategory: {data["name"]}')
                )
        ...
        for data in post_type_data:
            post_type, created = PostType.objects.get_or_create(
                name=data["name"], sub_category=data["sub_category"]
            )
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'Created PostType: {data["name"]}')
                )
            else:
                self.stdout.write(
                    self.style.NOTICE(f'Skipped PostType: {data["name"]}')
                )
        ...
        for data in boost_package_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'Created BoostPackage: {data["name"]}')
                )
            else:
                self.stdout.write(
                    self.style.NOTICE(f'Skipped BoostPackage: {data["name"]}')
                )
        ...
        for data in post_example_image_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'Created PostExampleImage: {data["name"]}')
                )
            else:
                self.stdout.write(
                    self.style.NOTICE(f'Skipped PostExampleImage: {data["name"]}')
                )
        ...
        image_group_data = [
            {
                "group_name": image_group_names[0],
                "image": post_example_images[0],
                "index": 1,
            },
            {
                "group_name": image_group_names[0],
                "image": post_example_images[1],
                "index": 2,
            },
            {
                "group_name": image_group_names[1],
                "image": post_example_images[3],
                "index": 1,
            },
        ]
        for data in image_group_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(
                        f'Created ImageGroup: {data["group_name"].name} - {data["image"].name}'
                    )
                )
            else:
                self.stdout.write(
                    self.style.NOTICE(
                        f'Skipped ImageGroup: {data["group_name"].name} - {data["image"].name}'
                    )
                )
        ...
        for item_list, required_length, list_name in validation_checks:
            if len(item_list) < required_length:
                raise ValueError(
                    f"Not enough {list_name} were created. Expected at least {required_length}, but got {len(item_list)}"
                )
            self.stdout.write(
                self.style.SUCCESS(f"Validated {list_name}: {len(item_list)} items")
            )
        ...
        for data in post_data:
            try:
                ...
                if created:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f'Created Post: {data["title"]}')
                    )
                else:
                    self.stdout.write(
                        self.style.NOTICE(f'Skipped Post: {data["title"]}')
                    )
                posts.append(post)
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f'Error creating post {data["title"]}: {str(e)}')
                )

        # Verify posts were created before proceeding
        if len(posts) == 0:
            raise ValueError(
                "No posts were created successfully. Cannot proceed with creating favorites."
            )
        ...
        if len(posts) >= 3:
            ...
            for data in favourite_data:
                favourite, created = Favourite.objects.get_or_create(
                    user=data["user"], post=data["post"]
                )
                if created:
                    self.stdout.write(
                        self.style.SUCCESS(
                            f'Created Favourite: User {data["user"].username} - Post {data["post"].title}'
                        )
                    )
                else:
                    self.stdout.write(
                        self.style.NOTICE(
                            f'Skipped Favourite: User {data["user"].username} - Post {data["post"].title}'
                        )
                    )
        else:
            self.stdout.write(
                self.style.WARNING(
                    "Not enough posts created to create favorites. Skipping favorite creation."
                )
            )
        ...
        for data in report_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f'Created Report: Post {data["post"].title}')
                )
            else:
                self.stdout.write(
                    self.style.NOTICE(f'Skipped Report: Post {data["post"].title}')
                )
        ...
        for data in report_chat_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f"Created ReportChat: Report #{report.id}")
                )
            else:
                self.stdout.write(
                    self.style.NOTICE(f"Skipped ReportChat: Report #{report.id}")
                )
        ...
        for data in boost_request_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(
                        f'Created BoostRequest: Post {data["post"].title}'
                    )
                )
            else:
                self.stdout.write(
                    self.style.NOTICE(
                        f'Skipped BoostRequest: Post {data["post"].title}'
                    )
                )
